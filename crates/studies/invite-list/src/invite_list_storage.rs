...
// Uses time-series approach to track safety changes over time

use buenotea_core::Result;
use crate::models::{InviteListRecord, InviteListInsert};
use buenotea_infrastructure::DatabaseClient;
use tracing::info;

/// Storage operations for invite list data
/// Stores historical time-series data in invite_list_history table
...
impl InviteListStorage {
    ...
    /// Store an invite list record in the database (time-series history)
    /// Each analysis is stored as a new record to track changes over time
    pub async fn store_invite_list_record(&self, record: &InviteListRecord) -> Result<i64> {
        info!("Storing invite list record for symbol: {} at {}", record.symbol, record.analysis_date);
        
        let url = format!("{}/rest/v1/invite_list_history", self.db_client.config().supabase_url);
    ...
    /// Store multiple invite list records in batch (time-series history)
    ...
    pub async fn update_invite_list_record(&self, record: &InviteListRecord) -> Result<()> {
        if let Some(id) = record.id {
            ...
            Err(buenotea_core::Error::DatabaseError("Cannot update record without ID".to_string()))
        }
    }
