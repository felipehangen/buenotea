...
use buenotea_invite_list::{InviteListCalculator, SP500Fetcher, InviteListStorage};
use buenotea_invite_list::models::{ApiConfig, InviteListRecord};
...
async fn main() -> Result<()> {
    ...
    // Use smaller batch sizes to avoid timeouts (reduced from 25 to 15)
    const BATCH_SIZE: usize = 15; // Optimized for reliability over speed
    ...
    for (batch_num, chunk) in all_records.chunks(BATCH_SIZE).enumerate() {
        ...
        // Delay between batches to avoid overwhelming Supabase (increased for reliability)
