// Example: Calculate TTS with regime analysis for AAPL and save to Supabase with ChatGPT analysis

use buenotea_regime::{RegimeCalculator, TTSResult};
use buenotea_database::{DatabaseClient, RegimeStorage};
...
async fn main() -> Result<()> {
    ...
    println!("🚀 Starting TTS with regime analysis for AAPL and saving to Supabase...\n");
    ...
    // For this example, we'll simulate the database connection
    // In production, you would use:
    // let db_client = DatabaseClient::new().await?;
    // let regime_storage = RegimeStorage::new(db_client);
    // let record_id = regime_storage.store_regime_analysis(&regime_record).await?;
    
    println!("📊 Database record prepared:");
    ...
    println!("💡 Note: Database storage requires Supabase connection configuration");
    println!("   Set SUPABASE_URL and SUPABASE_ANON_KEY environment variables");
...
mod tests {
    ...
    #[test]
    fn test_mock_tts_result_creation() {
        let result = create_mock_tts_result();
        assert_eq!(result.symbol, "AAPL");
        assert_eq!(result.tts_score, 0.35);
        assert_eq!(result.confidence_score, 0.82);
    }

    #[test]
    fn test_mock_api_tracking_creation() {
        let tracking = create_mock_api_tracking();
        assert_eq!(tracking.primary_api_source, "FMP");
        assert_eq!(tracking.price_data_points, 100);
        assert_eq!(tracking.current_price, 175.0);
    }
